...
public class GyroscopeControl : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Time.timeScale == 1 && GyroEnabled)
        {
            ...
            var rot = initialRotation * rawGyroRotation.rotation;
            _consoleTxt.text = $"rot: {rot.eulerAngles.ToString()} \n Quaternion: {Input.gyro.attitude.ToString()} \n Rot: {Input.gyro.attitude.eulerAngles.ToString()}";
        }
    }
